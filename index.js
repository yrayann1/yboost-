...
const TYPE_HABITATS = {
    Plante: 'Forets profondes',
    Feu: 'Zones volcaniques',
    Eau: 'Lacs et rivieres',
    Electrik: 'Plaines orageuses',
    Psy: 'Ruines mystiques',
    Poison: 'Marais toxiques',
    Insecte: 'Bois denses',
    Roche: 'Montagnes abruptes',
    Sol: 'Canyons arides',
    Vol: 'Falaises ventees',
    Combat: 'Arenes rocheuses',
    Fee: 'Clairieres lumineuses',
    Acier: 'Sites industriels',
    Normal: 'Prairies ouvertes',
    Dragon: 'Pics recules'
};
...
const getPokemonFullInfo = (pokemon) => {
    ...
    const statEntries = [
        ['Attaque', extra.attack],
        ['Defense', extra.defense],
        ['Vitesse', extra.speed]
    ];
    const dominantStat = statEntries.sort((left, right) => right[1] - left[1])[0][0];
    const baseTotal = (pokemon.hp || 0) + (pokemon.cp || 0) + (pokemon.power || 0) + extra.attack + extra.defense + extra.speed;
    const captureRate = clamp(65 - (pokemon.power || 0), 8, 60);

    let roleCombat = 'Polyvalent';
    if (dominantStat === 'Attaque') roleCombat = 'Finisseur offensif';
    else if (dominantStat === 'Defense') roleCombat = 'Tank defensif';
    else if (dominantStat === 'Vitesse') roleCombat = 'Eclaireur rapide';

    let temperament = 'Equilibre';
    if (extra.attack - extra.defense >= 14) temperament = 'Impulsif';
    else if (extra.defense - extra.attack >= 14) temperament = 'Protecteur';
    else if (extra.speed >= 78) temperament = 'Nerveux';

    const genre = pokemon.id % 7 === 0
        ? 'Asexue'
        : (pokemon.id % 2 === 0 ? '50% male / 50% femelle' : '62% male / 38% femelle');
    const niveauConseille = clamp(Math.round((pokemon.power || 0) * 1.7 + (pokemon.cp || 0) * 6), 8, 90);

    let difficulte = 'Accessible';
    if (captureRate <= 15) difficulte = 'Extreme';
    else if (captureRate <= 30) difficulte = 'Elevee';
    else if (captureRate <= 45) difficulte = 'Moyenne';
    ...
    return {
        ...
        habitat: TYPE_HABITATS[firstType] || 'Zone inconnue',
        roleCombat,
        temperament,
        genre,
        niveauConseille,
        ...
        baseTotal,
        dominantStat,
        mouvementSignature: TYPE_SIGNATURE_MOVES[firstType] || 'Charge',
        talents,
...
<head>
    ...
    <style>
        ...
        .card::before {
            ...
            inset: 0 0 auto;
            height: 5px;
            background: linear-gradient(90deg, var(--brand), #6366f1, var(--brand-alt));
        }
        .card::after {
            content: '';
            position: absolute;
        ...
        .power-bar span {
            ...
            background: linear-gradient(90deg, #c7d2fe 0%, #6366f1 50%, #0284c7 100%);
        }
        .detail {
            display: grid;
...
const renderHomePage = () => {
    ...
    return renderLayout('Accueil Pokedex', `
        <section class="home-section home-hero">
            <div>
                <h1 class="home-title">Explore ton Pokedex</h1>
                <p class="home-lead">Recherche, compare et découvre les stats de chaque Pokémon dans une interface premium.</p>
                <div class="home-cta">
            ...
            <div class="home-hero-right">
                ...
                <div class="badge">Version Pro UI</div>
...
const renderCardFull = (pokemon) => {
    ...
    <a class="card" href="/api/pokemons?id=${pokemon.id}">
        ...
        <p class="card-extra">${escapeHtml(full.roleCombat)} • ${escapeHtml(full.habitat)}</p>
        <div class="card-foot">
        ...
        <section class="detail">
            ...
            <div class="panel">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item"><small>Habitat</small><strong>${escapeHtml(full.habitat)}</strong></div>
                    <div class="info-item"><small>Rôle</small><strong>${escapeHtml(full.roleCombat)}</strong></div>
                    <div class="info-item"><small>Tempérament</small><strong>${escapeHtml(full.temperament)}</strong></div>
                    <div class="info-item"><small>Genre</small><strong>${escapeHtml(full.genre)}</strong></div>
                    <div class="info-item"><small>Niveau conseillé</small><strong>${full.niveauConseille}</strong></div>
                    <div class="info-item"><small>Difficulté</small><strong>${escapeHtml(full.difficulte)}</strong></div>
                    <div class="info-item"><small>Total stats</small><strong>${full.baseTotal}</strong></div>
                    <div class="info-item"><small>Stat dominante</small><strong>${escapeHtml(full.dominantStat)}</strong></div>
                    <div class="info-item"><small>Mouvement signature</small><strong>${escapeHtml(full.mouvementSignature)}</strong></div>
